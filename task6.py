#Я взял это из интернета, потому что застрял на этом задании. Когда увидел этот вариант, в принципе понял, как это реализовано.
...
control = None
while True:
    control = input("For quit press 'Quit', for continue press 'Enter', for analytics press 'A'").upper() 
    if control == 'QUIT':
        break
    num += 1
    if control == 'A':
